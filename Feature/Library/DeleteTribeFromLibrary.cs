...
namespace HAS.Content.Feature.Library
{
    ...
    {
        public class DeleteTribeFromLibraryCommand : IRequest<bool>, ICommandEvent
        {
            public string HubId { get; set; }
            public string LibraryId { get; set; }
            public string TribeId { get; set; }

            public DeleteTribeFromLibraryCommand(string hubId, string libraryId, string tribeId)
            {
                HubId = hubId;
                LibraryId = libraryId;
                TribeId = tribeId;
        ...
        public class DeleteTribeFromLibraryCommandHandler : IRequestHandler<DeleteTribeFromLibraryCommand, bool>
        {
            ...
            public DeleteTribeFromLibraryCommandHandler(LibraryContext db, IMediator mediator)
            {
                _db = db;
                _mediator = mediator;
            ...
            public async Task<bool> Handle(DeleteTribeFromLibraryCommand cmd, CancellationToken cancellationToken)
            {
                ...
                if (hub.Handle(cmd))
                {
                    ...
                    try
                    {
                        ...
                        return !update.Libraries.Where(x => x.Id == ObjectId.Parse(cmd.LibraryId)).FirstOrDefault().Tribes.Any(x => x.Id == ObjectId.Parse(cmd.TribeId));
